...
let signup_email = (email = "", pattern = "", res) => {
...
<div class="container-fluid text-center p-0 m-0 border-0">
    <h1 class="p-5">Verify email ${pattern == "signup" ? "for sign up" :
            pattern == "resetpwd" ? "for reset password" : ""
        }</h1>
    <hr>
    <div class="pt-2 pb-5">
        <p>In order we are getting verified account ${pattern == "signup" ? "for new account" :
            pattern == "resetpwd" ? "for reset account password" : ""
        }
        with <b>${email}</b> before expired in 30 minutes by click the button below </p>
    </div>
    <div class="mx-auto">
        <a href="${process.env.BACKEND_HOST}/api/authentication/verify?path=${pattern == "signup" ? "login" :
        pattern == "resetpwd" ? "resetpwd" : "" }&email_phone=${email}&vf_token=${generateVerifyToken(res)}" class="btn btn-success text-center">Verify my email</a>
    </div>
</div>
`
}
